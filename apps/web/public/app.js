...
const tutorialManager=(()=>{
  ...
  async function show(section,index){const s=sections[section],step=s?.steps[index];if(!step)return finishSection(section);if(location.pathname!==s.route)return start(section,index,read().full,read().fullIndex||0);if(step[3]==='createTrade')showTradeTab('create');const target=await waitFor(step[0]);if(!target)return fallback(section,index);target.closest('.site-nav')?.setAttribute('data-tour-nav','true');if(innerWidth<900&&target.closest('#nav-links'))$('nav-links')?.classList.add('open');target.scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});await new Promise(r=>setTimeout(r,240));if(!visible(target))return fallback(section,index);paint(section,index,target)}
