...
async function bindAdmin(){
  ...
  try{
    const [dashboard,smartRooms]=await Promise.all([api('/api/web-admin/dashboard'),api('/api/web-admin/smart-rooms')]);
  ...
  $('admin-settings-form').onsubmit=async event=>{
    ...
    const body={
      ...
      dmNotificationsEnabled:$('setting-dm-enabled').checked,quickMatchEnabled:$('setting-quick-match').checked,autoSmartRoomsEnabled:$('setting-auto-smart-rooms').checked,ratingsEnabled:$('setting-ratings').checked,reportsEnabled:$('setting-reports').checked,autoCreateRoomChannels:$('setting-auto-channels').checked,aiChatEnabled:$('setting-ai-enabled').checked,
