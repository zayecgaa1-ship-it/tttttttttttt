...
async function boot() {
  renderShell();
  if (page === "commands") { await new Promise(resolve=>setTimeout(resolve,0)); await tutorialManager.autoStart(); return; }
...
const tutorialManager=(()=>{
  ...
  let cleanupFns=[],activeTarget=null,raf=0;
  ...
  const clean=()=>{cancelAnimationFrame(raf);cleanupFns.splice(0).forEach(fn=>fn());document.getElementById('zark-tutorial-v4')?.remove();document.querySelectorAll('[data-tour-active]').forEach(n=>n.removeAttribute('data-tour-active'));document.querySelectorAll('[data-tour-nav]').forEach(n=>n.removeAttribute('data-tour-nav'));activeTarget=null};
  ...
  async function autoStart(){const saved=read();if(await completed())return;if(saved.pausedVersion===version)return;if(saved.status==='active'&&saved.section)return show(saved.section,saved.step||0);openCenter(true)}
  ...
  async function show(section,index){const s=sections[section],step=s?.steps[index];if(!step)return finishSection(section);if(location.pathname!==s.route)return start(section,index,read().full,read().fullIndex||0);if(step[3]==='createTrade')showTradeTab('create');if(step[0]==='#nav-links'&&innerWidth<=1200){$('nav-links')?.classList.add('open');$('mobile-menu')?.setAttribute('aria-expanded','true');}const target=await waitFor(step[0]);if(!target)return fallback(section,index);target.closest('.site-nav')?.setAttribute('data-tour-nav','true');if(innerWidth<=1200&&target.closest('#nav-links'))$('nav-links')?.classList.add('open');target.scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});await new Promise(r=>setTimeout(r,240));if(!visible(target))return fallback(section,index);paint(section,index,target)}
