...
const links = [['home','/','الرئيسية'],['lfg','/lfg.html','LFG'],['games','/games.html','الألعاب'],['trade','/trade.html','Trade'],['leaderboard','/leaderboard.html','التصنيف'],['profile','/profile.html','ملفي'],['reports','/reports.html','الدعم'],['commands','/commands.html','البوت']];
...
const tutorialManager=(()=>{
  ...
  let cleanupFns=[],activeTarget=null,raf=0,tourEpoch=0;
  const read=()=>{try{return JSON.parse(localStorage.getItem(key)||'{}')}catch{return {}}};
  const write=patch=>{const value={...read(),...patch,version};localStorage.setItem(key,JSON.stringify(value));return value};
  ...
  async function autoStart(){const saved=read();if(saved.status==='active'&&sections[saved.section])return show(saved.section,saved.step||0);if(await completed())return;if(saved.pausedVersion===version)return;openCenter(true)}
  ...
  function openCenter(first=false){clean();const saved=read(),done=saved.completedSections||{},current=Object.entries(sections).find(([,s])=>s.route===location.pathname)?.[0];const layer=document.createElement('section');layer.id='zark-tutorial-v4';layer.className='tour-invite tutorial-center tutorial-v4-center';layer.setAttribute('role','dialog');layer.setAttribute('aria-modal','true');layer.innerHTML=`<article><button class="onboarding-skip" data-close>إغلاق ×</button><span>📚</span><h2>دليل استخدام Zark</h2><p>من أول غرفة إلى متجر الولاء وVIP والنكت والميمز. اختر ما تحتاجه أو اتبع الجولة خطوة بخطوة. تستطيع اختيار قسم أو بدء الجولة الكاملة.</p><button class="tutorial-full-start" data-full><i>🚀</i><span><b>ابدأ الشرح الكامل</b><small>يمكنك التنقل بالسابق والتالي، ويُحفظ تقدمك.</small></span></button><div class="tutorial-section-title">الأقسام</div><div class="tutorial-section-grid">${Object.entries(sections).map(([id,s])=>`<button data-section="${id}"><i>${s.icon}</i><b>${s.title}</b><small>${done[id]?'✅ مكتمل':`${s.steps.length} خطوات`}</small></button>`).join('')}</div><div class="tutorial-center-actions">${current?'<button class="button primary" data-page>اشرح هذه الصفحة</button>':''}<button class="button ghost" data-later>${first?'لاحقًا':'إلغاء'}</button></div></article>`;const pause=()=>{write({status:'paused',pausedVersion:version});clean()};layer.querySelector('[data-close]').onclick=pause;layer.querySelector('[data-later]').onclick=pause;layer.querySelector('[data-full]').onclick=()=>start('basics',0,true,0);layer.querySelector('[data-page]')?.addEventListener('click',()=>start(current));layer.querySelectorAll('[data-section]').forEach(b=>b.onclick=()=>start(b.dataset.section));document.body.append(layer);layer.querySelector('[data-full]').focus();const onKey=e=>{if(e.key==='Escape'){e.preventDefault();pause();}if(e.key==='Tab'){const nodes=[...layer.querySelectorAll('button,a[href]')].filter(n=>!n.disabled);const first=nodes[0],last=nodes.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}}};layer.addEventListener('keydown',onKey)}
  function start(section,index=0,full=false,fullIndex=0){const s=sections[section];if(!s)return;write({status:'active',section,step:index,full,fullIndex,pausedVersion:0});clean();if(location.pathname!==s.route){location.assign(s.route);return}show(section,index)}
  async function show(section,index){const epoch=tourEpoch;const s=sections[section],step=s?.steps[index];if(!s)return openCenter();if(!step)return finishSection(section);if(location.pathname!==s.route)return start(section,index,read().full,read().fullIndex||0);if(step[3]==='createTrade')showTradeTab('create');if(section==='lfg'&&index<=4)$('open-create-room')?.click();else if(section==='lfg')$('close-create-room')?.click();if(step[0]==='#nav-links'&&innerWidth<=1200){$('nav-links')?.classList.add('open');$('mobile-menu')?.setAttribute('aria-expanded','true');}const target=await waitFor(step[0]);if(epoch!==tourEpoch)return;if(!target)return fallback(section,index);target.closest('.site-nav')?.setAttribute('data-tour-nav','true');if(innerWidth<=1200&&target.closest('#nav-links'))$('nav-links')?.classList.add('open');target.scrollIntoView({block:'center',behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});await new Promise(r=>setTimeout(r,240));if(epoch!==tourEpoch)return;if(!visible(target))return fallback(section,index);paint(section,index,target)}
  ...
  function waitFor(selector){return new Promise(resolve=>{const until=Date.now()+6000;const check=()=>{const node=document.querySelector(selector);if(visible(node))return resolve(node);if(Date.now()>until)return resolve(null);requestAnimationFrame(check)};check()})}
  function paint(section,index,target){clean();activeTarget=target;target.closest('.site-nav')?.setAttribute('data-tour-nav','true');target.setAttribute('data-tour-active','true');const s=sections[section],step=s.steps[index],layer=document.createElement('section');layer.id='zark-tutorial-v4';layer.className='product-tour-layer tutorial-v4-layer';layer.innerHTML=`<div class="tour-spotlight"></div><aside class="tour-tooltip"><button class="tour-close" aria-label="إيقاف الشرح" data-pause>×</button><small>${s.title} · ${index+1}/${s.steps.length}</small><h2>${step[1]}</h2><p>${step[2]}</p><div><button class="button ghost small" data-back ${index===0?'disabled':''}>السابق</button><button class="button ghost small" data-center>الأقسام</button><button class="button primary small" data-next>${index===s.steps.length-1?'إنهاء القسم':'التالي'}</button></div></aside>`;document.body.append(layer);const move=()=>{cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>position(layer,target))};const keyboard=e=>{if(e.key==='Escape'){write({status:'paused',pausedVersion:version});clean()}else if(e.key==='ArrowLeft'){e.preventDefault();next(section,index)}else if(e.key==='ArrowRight'){e.preventDefault();previous(section,index)}};addEventListener('resize',move,{passive:true});addEventListener('scroll',move,true);document.addEventListener('keydown',keyboard);const observer=new ResizeObserver(move);observer.observe(target);cleanupFns.push(()=>removeEventListener('resize',move),()=>removeEventListener('scroll',move,true),()=>document.removeEventListener('keydown',keyboard),()=>observer.disconnect());layer.querySelector('[data-pause]').onclick=()=>{write({status:'paused',pausedVersion:version});clean()};layer.querySelector('[data-back]').onclick=()=>previous(section,index);layer.querySelector('[data-center]').onclick=()=>openCenter();layer.querySelector('[data-next]').onclick=()=>next(section,index);move();layer.querySelector('[data-next]').focus();console.info('tutorial_step_viewed',section,step[1])}
  function position(layer,target){if(!visible(target))return fallback(read().section,read().step||0);const r=target.getBoundingClientRect(),spot=layer.querySelector('.tour-spotlight'),tip=layer.querySelector('.tour-tooltip'),pad=8,vw=innerWidth,vh=innerHeight;spot.style.cssText=`top:${Math.max(4,r.top-pad)}px;left:${Math.max(4,r.left-pad)}px;width:${Math.min(vw-8,r.width+pad*2)}px;height:${Math.min(vh-8,r.height+pad*2)}px`;if(vw<=700){tip.classList.add('tour-tooltip-mobile');return}tip.classList.remove('tour-tooltip-mobile');const box=tip.getBoundingClientRect(),gap=16,candidates=[[r.left+(r.width-box.width)/2,r.bottom+gap],[r.left+(r.width-box.width)/2,r.top-gap-box.height],[r.right+gap,r.top],[r.left-gap-box.width,r.top]];const found=candidates.find(([x,y])=>x>=12&&y>=12&&x+box.width<=vw-12&&y+box.height<=vh-12)||candidates[0];tip.style.left=`${Math.max(12,Math.min(vw-box.width-12,found[0]))}px`;tip.style.top=`${Math.max(12,Math.min(vh-box.height-12,found[1]))}px`}
  function previous(section,index){if(index>0)start(section,index-1,read().full,read().fullIndex||0)}
  ...
  async function finishSection(section){const state=read(),done={...(state.completedSections||{}),[section]:true};if(state.full){const keys=Object.keys(sections),nextIndex=(state.fullIndex||0)+1;if(nextIndex<keys.length){write({completedSections:done,fullIndex:nextIndex});return start(keys[nextIndex],0,true,nextIndex)}write({completed:true,status:'completed',completedSections:done,full:false});await account(true);return completeDialog()}write({status:'idle',completedSections:done});openCenter()}
  function fallback(section,index){clean();const layer=document.createElement('section');layer.id='zark-tutorial-v4';layer.className='tour-invite tutorial-v4-fallback';layer.innerHTML=`<article><span>📍</span><h2>ننتقل للخطوة التالية</h2><p>هذا العنصر غير متاح الآن أو ما زال قيد التحميل؛ لن نُظهر تلميحًا في مكان خاطئ.</p><button class="button primary" data-next>التالي</button></article>`;layer.querySelector('[data-next]').onclick=()=>next(section,index);document.body.append(layer);layer.querySelector('[data-next]').focus()}
  function completeDialog(){clean();const layer=document.createElement('section');layer.id='zark-tutorial-v4';layer.className='tour-invite tutorial-v4-fallback';layer.innerHTML='<article><span>🎉</span><h2>اكتمل الشرح</h2><p>لن يظهر تلقائيًا مرة أخرى. تستطيع إعادة تشغيله من ملفك أو زر الشرح.</p><button class="button primary" data-close>تم</button></article>';layer.querySelector('[data-close]').onclick=clean;document.body.append(layer);layer.querySelector('[data-close]').focus()}
