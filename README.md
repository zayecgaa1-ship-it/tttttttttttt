...
- الكتالوج يضم 42 لعبة، بما فيها الإيموجي والأفلام والمسلسلات والموسيقى وشعارات السيارات والشركات.
- أعداد الأسئلة تمثل الأسئلة الأصلية، مع تمييز الصور المختلفة. أُلغي تكرار السؤال تحت عناوين زخرفية.
- الاختيار يفضّل الأسئلة غير المستخدمة في القناة، ثم يعود إلى الأقدم عند استنفاد البنك. أسئلة الإدارة تشارك في نفس آلية الاختيار.
- الأرقام تُطابق بدقة مع دعم الأرقام العربية، ولعبة أسرع كتابة تتطلب النص كما هو.
- الموقع يدعم حفظ المفضلة على الجهاز، تصنيف الألعاب وترتيبها، وعرض الأمر والوقت وعدد الأسئلة.
- مسار التشغيل الحالي يطبق تحديث إعادة الألعاب القديمة ضمن `db:prepare` قبل تشغيل الخدمات.

اختبارات التحديث، دون إرسال رسائل إلى Discord:

```bash
npm run verify
node scripts/web-design-smoke.mjs --games-hub
node scripts/lfg-platform-db-isolated.mjs scripts/games-engine-db-smoke.ts
```

الفحص الأخير يحتاج PostgreSQL محليًا، وينشئ مخططًا مؤقتًا ويزيله بعد الاختبار. فحص المتصفح يستخدم Edge وبيانات تجريبية.
# لوحة الاهتمامات ومحاولة صح أو خطأ

استخدم `/setup` داخل روم Discord لنشر لوحة دائمة لاختيار الألعاب والمنصة والإشعارات. نشر اللوحة يحتاج صلاحية **إدارة السيرفر**؛ يستطيع كل لاعب فتح إعداداته الخاصة من الزر دون تغيير لوحة الآخرين.
